...
const Task:FC<taskType> = ({task}) =>{
    ...
    const [{monitorId},drag] = useDrag({
        ...
        begin:()=> {dispatch(actions.setActiveTask(task))},
        end:(item,monitor)=>{
            if(!monitor.didDrop()){
    ...
    return (
        <div className="c-board__card c-card" ref={drag} data-testid="task" data-monitorid={monitorId}>
                <h3 className="c-card__title">{task.title}</h3>
                <p className="c-card__detail">
                    {task.detail}
                </p>
        </div>
    );
